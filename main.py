...
def main(args):
    ...
    if args.sgd:
        ...
        optimizer = torch.optim.AdamW(param_dicts, lr=args.lr,
                                      weight_decay=args.weight_decay)
    lr_scheduler = torch. optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40, 60])
    ...
    if args.pretrained:
        ...
        for key in checkpoint['model'].keys():
            print(f"Key: {key}, Shape: {checkpoint['model'][key].shape}")

        target_shape = torch.Size([300, 512])
        for key, value in checkpoint['model'].items():
            if value.shape == target_shape:
                print(f"Key with shape {target_shape}: {key}")
        
        checkpoint['model'].pop('query_embed.weight')
        
        # for i in range(6):
        #     checkpoint['model'].pop('class_embed.' + str(i) + '.weight')
        #     checkpoint['model'].pop('class_embed.' + str(i) + '.bias')    

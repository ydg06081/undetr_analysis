...
class DeformableTransformer(nn.Module):
    ...
    def draw_feamap(self, memory, level_start_index, spatial_shapes, img):
    ...
    #각 레벨의 feature map을 가져옴
        ...
        for i, feat in enumerate(feats):
            feat_map = torch.mean(feat[0], dim=2, keepdim=True).detach()
            feat_map = feat_map.cpu().numpy()
            feat_map = feat_map / np.max(feat_map)
            feat_map = cv2.resize(feat_map, (img.shape[3], img.shape[2]), interpolation=cv2.INTER_LINEAR)
            if i == 0:
                attn_map = feat_map
            else:
                attn_map += feat_map
        ...
        plt.imshow(attn_map)
        plt.axis('off')
        plt.savefig('./feature_map/fmap.png', bbox_inches="tight", pad_inches=0)
        plt.close()
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, epoch=0):
...
#특징맵은 4개니깐 
        ...
        if self.two_stage:
            ...
            topk_proposals = topk_proposals.squeeze(2)

...
def save_result(img_name, boxes, ori_prob,  objs, img_size):
    ...
    # img_size [h ,w, c]
        ...
        if len(boxes.shape) == 1:
            ...
            # if ori_prob[0][ori_prob[0].argmax()].item() > 0.5:  
            ...
            for index, coordinate in enumerate(boxes):
                ...
                # if ori_prob[index][ori_prob[index].argmax()].item() > 0.7:
...
def plot_result(pil_img, prob, boxes, objs, save_name=None, imshow=False, imwrite=False, save_txt=False):    
    ...
    if save_txt:
        ...
        save_result(save_name, boxes, prob, objs, img_size)
        imshow = False
        imwrite = False
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        plot_result(im, scores, boxes, objs, save_name=file, imshow=False, imwrite=False, save_txt=True)
